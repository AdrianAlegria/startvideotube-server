...
# ─── App ───────────────────────────────────────────────────────────────────────
app = FastAPI(title="StartVideoTube", version="5.3.2")
...
def download_thumbnail(thumbnail_url: str, image_path: str) -> bool:
    """
    Descarga la imagen de fondo desde una URL.
...
def render_video(
    ...
    """
    # Calcular zoom para Ken Burns (de 1.0 a 1.05 durante toda la duración)
    # zoompan requiere duración en frames (25fps)
    total_frames = int(duration * 25)

    # Filtro de video con Ken Burns
    vf_filter = (
        f"zoompan=z='min(zoom+0.0002,1.05)':d={total_frames}:s=1080x1920:fps=25,"
    ...
    if result.returncode != 0:
        ...
        raise HTTPException(status_code=500, detail=f"Error ffmpeg: {result.stderr[-300:]}")
